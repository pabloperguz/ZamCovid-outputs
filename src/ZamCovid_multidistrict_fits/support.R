...
pars_info_single <- function(district, info) {
  
  cols <- c("district", "include", "name", "initial", "max", "integer")
  stopifnot(all(cols %in% colnames(info)))
  
  if (!(district %in% info$district)) {
    stop(sprintf("Did not find district '%s' in parameter info", district))
  }
  info <- info[info$district == district & info$include, ]
  info <- info[setdiff(names(info), c("district", "include"))]
  rownames(info) <- NULL
  info
}


pars_prior_single <- function(district, info, prior) {
  prior_cols <- c("district", "type", "name", "gamma_scale", "gamma_shape",
                  "beta_shape1", "beta_shape2")
  stopifnot(all(prior_cols %in% colnames(prior)))
  prior <- prior[prior$district == district & prior$name %in% info$name, ]
  prior <- prior[setdiff(names(prior), "district")]
  rownames(prior) <- NULL
  prior
}


pars_proposal_single <- function(district, info, proposal, kernel_scaling) {
  proposal <- proposal[proposal$district == district, ]
...
fit_pars_load <- function(path, district, assumptions, short_run, deterministic) {
  ...
  info <- pars_info_single(district, parameters$info)
  prior <- pars_prior_single(district, info, parameters$prior)
  proposal <- pars_proposal_single(district, info, parameters$proposal,
                                   kernel_scaling)
  
  dat <- load_transform(path, district, assumptions)
  base <- dat$base
...
parse_data <- function(dat, district, fit_sero = FALSE, fit_deaths = FALSE,
                       fit_cases = FALSE, sero_by_age = FALSE, deaths_by_age = FALSE) {
  # browser()
  ...
  expected <- c("date", "date_string", "district",
                "deaths_all", "sero_tot_all",
                "sero_pos_all", "sero_pos_over15", "sero_tot_over15",
                "pcr_pos_all", "pcr_tot_all",
                # Age-disaggregated columns
                sero_age_bands, deaths_age_bands,
  ...
  dat <- dat[dat$district == district, ]
  dat <- dat[as.Date(dat$date_string) <= as.Date(cull_date), ]
  ...
  if (fit_sero) {
    ...
    if (sero_by_age) {
      ...
      dat[, c("sero_pos_all", "sero_tot_all")] <- NA_integer_
    } else {
      stopifnot(!all(is.na(dat$sero_pos_all)) && !all(is.na(dat$sero_tot_all)))
  ...
  # We'll ignore deaths before 2020-03-15 to avoid inferring a much to earlier
  # seeding of the epidemic
  death_cols <- grep("deaths", colnames(dat), value = TRUE)
  dat[as.Date(dat$date_string) < as.Date("2020-03-15"), death_cols] <- NA_integer_
  
  
  if (fit_cases) {
  
    stopifnot(!all(is.na(dat$pcr_tot_all)) && !all(is.na(dat$pcr_pos_all)))
    
  } else {
    dat[, c("pcr_pos_all", "pcr_tot_all")] <- NA_integer_
  }
