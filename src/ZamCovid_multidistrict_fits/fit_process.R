...
fit_parameters <- function(samples, parameters) {
  ...
  keep <- function(x, district) {
    is.na(x) | x %in% district
  }
  
  district <- samples$info$district
  info <- parameters$raw$info[keep(parameters$raw$info$district, district), ]
  rownames(info) <- NULL
  ...
  proposal <- data_frame(district = samples$info$district,
                         name = colnames(covariance),
                         covariance)
  
  prior <- parameters$prior
  prior$district <- samples$info$district
  prior <- prior[, c("district", setdiff(names(prior), "district"))]
  rownames(prior) <- NULL
